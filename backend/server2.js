...
const statementStream = await axios({
    ...
    statementStream.data.on('end', () => {
      ...
      const combinedTail = `,"analytics":${JSON.stringify(analyticsRes.data)},"document":{"downloadUrl":"/internal/document?sizeMb=50"}}`
      ...
      logClientResponse({
        requestId: req.context.requestId,
        payload: { statement: '[STREAMED]', analytics: analyticsRes.data, document: { downloadUrl: '/internal/document?sizeMb=50' } },
        status: 'SUCCESS'
      })
    })
